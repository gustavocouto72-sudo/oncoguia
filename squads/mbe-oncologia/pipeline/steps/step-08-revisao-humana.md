...
Schema de cada item de `decisoes[]`: `{regimen_id, content_hash, hash_atual, decisao (aprovado | contestado | ajuste_solicitado), natureza (dado | clinico | null), acao (refutar | excluir | corrigir_referencia | ajustar_elegibilidade | manter_anotar | outro | null), acao_detalhe, aplicada_em, estado_intake, triagem_manual, eixo, justificativa, reprocessar, correcao, revisor, data}`. (`remover`, nome antigo de `refutar`, foi migrado no banco — exports novos nunca o trazem; se aparecer num arquivo antigo, tratar como `refutar`.) Usar a decisão **mais recente** por `regimen_id`; se `content_hash` ≠ hash atual do regime, o parecer é de uma versão anterior — registrar, mas tratar como re-revisão pendente.

**`estado_intake` diz o que ainda há para fazer** — ter `acao` NÃO significa que ela já rodou; quem atesta execução é `aplicada_em` (data do run que executou). Os estados são exclusivos, nesta precedência:
- `aguardando_re_revisao` — `hash_atual` ≠ `content_hash`: o parecer é de uma versão anterior (inclusive quando foi a própria execução que mudou o regime). **Não re-executar**: registrar e devolver ao revisor, que decide sobre a versão nova.
- `pendente_triagem` — crítica sem `acao` (legado; mesmo conjunto de `triagem_manual: true`) → passo 3.
- `triada_aplicada` — `acao` + `aplicada_em`: **ciclo fechado, nada a refazer**. Reingerir seria re-executar o que já foi executado.
- `triada_pendente_execucao` — `acao` sem `aplicada_em`: **é a fila de trabalho real** deste step (tipicamente `acao: "outro"`, que nunca roteia automático).
...
5. **Fechar o ciclo no banco**: para cada decisão que este run de fato EXECUTOU, gravar de volta em `revisoes` a `acao` confirmada na triagem e `aplicada_em` = data do run. Sem isso a decisão volta como pendente no export seguinte e seria re-executada. O que ficou em `outro` sem execução (`decisao_revisao = "triagem_manual"`) recebe a `acao`, mas **não** recebe `aplicada_em` — é a fila manual. Nada do parecer do revisor (decisão, justificativa, natureza, revisor, data) é reescrito.
