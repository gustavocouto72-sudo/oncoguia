...
export class RetornosService {
  ...
  //
  // TODO (integração): o tipo 'autorizacao' entra aqui quando o módulo de autorização
  // existir — a mescla já é genérica (basta empurrar itens {tipo:'autorizacao', data, …}
  // em `itens` antes do sort). Hoje essa fonte não existe e a trilha traz só os dois tipos.
  async trilha(pacienteId: number) {
    ...
    itens.sort((x, y) =>
      (x._dia < y._dia ? -1 : x._dia > y._dia ? 1 : 0) || x._instante - y._instante || x.id - y.id);
