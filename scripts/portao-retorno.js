...
//
// Uso: node scripts/portao-retorno.js   (exige app em 5176 e API em 3007 — instâncias
// EFÊMERAS de verificação deste worktree; 3005/5173 são do backend principal.)
...
const APP = process.env.PORTAO_APP || 'http://localhost:5176/index.html';
const API = process.env.PORTAO_API || 'http://localhost:3007/api';
...
(async () => {
  ...
  try {
    ...
    const ridEscolhido = await page.evaluate(() => {
      const b = document.querySelector('#reaval-protos .sel-btn');
      return b.getAttribute('onclick').split("'")[1];
    });
    await page.click('#reaval-protos .sel-btn');
